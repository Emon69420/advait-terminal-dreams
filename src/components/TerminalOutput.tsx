...
const TerminalOutput: React.FC<TerminalOutputProps> = ({ output }) => {
  ...
  switch (output.type) {
    ...
    case 'ascii':
      return (
        <pre className={`mb-2 font-mono text-terminal-accent whitespace-pre ${isMobile ? 'text-[0.3rem] sm:text-xs' : 'text-xs sm:text-sm'} ${isMobile ? 'overflow-x-scroll' : 'overflow-x-auto'}`}>
          {displayedText}
        </pre>
      );
